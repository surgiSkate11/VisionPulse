...
class NotionIconPicker {
        ...
        return `
            <div class="notion-picker-tab-content" data-content="upload" style="display: none;">
                <div class="notion-picker-upload-area" data-upload-area>
                    <div class="notion-picker-upload-icon">📁</div>
        ...
        tabs.forEach(tab => {
            tab.addEventListener('click', (e) => {
                ...
                if (activeSearch) {
                    setTimeout(() => {
                        activeSearch.focus();
                        activeSearch.value = ''; // Limpiar búsqueda anterior
                    }, 50);
                }
    ...
    bindSearchEvents() {
        ...
        searches.forEach(search => {
            ...
            search.addEventListener('input', (e) => {
                ...
                }, 100); // Debounce reducido para búsqueda más rápida
            });
    ...
    loadIcons() {
        ...
        const iconCategories = {
            'Educación': ['fas fa-graduation-cap', 'fas fa-book', 'fas fa-pencil-alt', 'fas fa-calculator', 'fas fa-microscope', 'fas fa-flask', 'fas fa-atom', 'fas fa-dna', 'fas fa-brain', 'fas fa-lightbulb'],
            'Tecnología': ['fas fa-laptop', 'fas fa-mobile-alt', 'fas fa-code', 'fas fa-database', 'fas fa-server', 'fas fa-wifi', 'fas fa-bluetooth', 'fas fa-usb', 'fas fa-microchip', 'fas fa-robot'],
            'Negocios': ['fas fa-briefcase', 'fas fa-chart-line', 'fas fa-chart-pie', 'fas fa-dollar-sign', 'fas fa-handshake', 'fas fa-bullseye', 'fas fa-rocket', 'fas fa-trophy', 'fas fa-medal', 'fas fa-star'],
            'Comunicación': ['fas fa-phone', 'fas fa-envelope', 'fas fa-comment', 'fas fa-comments', 'fas fa-video', 'fas fa-microphone', 'fas fa-headphones', 'fas fa-bullhorn', 'fas fa-rss', 'fas fa-broadcast-tower'],
            'Transporte': ['fas fa-car', 'fas fa-bus', 'fas fa-train', 'fas fa-plane', 'fas fa-ship', 'fas fa-bicycle', 'fas fa-motorcycle', 'fas fa-truck', 'fas fa-taxi', 'fas fa-helicopter'],
            'Herramientas': ['fas fa-wrench', 'fas fa-hammer', 'fas fa-screwdriver', 'fas fa-toolbox', 'fas fa-cog', 'fas fa-nut-bolt', 'fas fa-drill', 'fas fa-saw', 'fas fa-ruler', 'fas fa-level']
        };
        ...
        Object.entries(iconCategories).forEach(([category, icons]) => {
            ...
            section.innerHTML = `
                ...
                <div class="notion-picker-grid">
                    ${icons.map(icon => `
                        <button type="button" class="notion-picker-item" data-icon="${icon}" title="${icon}">
                            <i class="${icon}"></i>
                        </button>
                    `).join('')}
                </div>
    ...
    selectIcon(iconClass) {
        // Para iconos, podríamos mostrar el HTML o solo la clase
        this.input.value = `<i class="${iconClass}"></i>`;
        this.triggerChange();
        this.close();
    }
    ...
    handleFileUpload(file) {
        ...
        reader.onload = (e) => {
            ...
            this.input.value = e.target.result; // Data URL de la imagen
    ...
    filterItems(type, query) {
        ...
        items.forEach(item => {
            ...
            if (query) {
                ...
                if (type === 'emojis') {
                    // Para emojis, buscar por el emoji mismo y por el nombre asignado
                    const emoji = item.dataset.emoji || item.textContent.trim();
                    const name = (item.dataset.name || '').toLowerCase();
                    
                    matches = emoji.includes(searchQuery) || name.includes(searchQuery);
                } else if (type === 'icons') {
                    // Para iconos, buscar en la clase y en palabras clave
                    const iconClass = (item.dataset.icon || '').toLowerCase();
                    const keywords = this.getIconKeywords(iconClass);
                    matches = iconClass.includes(searchQuery) ||
        ...
        // Mostrar/ocultar secciones según elementos visibles
    ...
    getIconKeywords(iconClass) {
        const keywordMap = {
            'graduation-cap': ['educacion', 'titulo', 'universidad', 'estudio'],
            'book': ['libro', 'lectura', 'manual', 'guia'],
            'pencil-alt': ['escribir', 'editar', 'lapiz'],
            'calculator': ['matematicas', 'numeros', 'calcular'],
            'laptop': ['computadora', 'ordenador', 'tecnologia'],
            'mobile-alt': ['telefono', 'movil', 'celular'],
            'envelope': ['correo', 'email', 'mensaje'],
    ...
    triggerChange() {
        // Disparar evento change en el input
    ...
    open() {
        ...
        // Focus en el search del tab activo
        setTimeout(() => {
