...
# Parámetros optimizados para OJOS ASIÁTICOS (apertura más estrecha)
EAR_THRESH = 0.24     # Umbral MÁS BAJO - ojos asiáticos tienen EAR naturalmente menor
NUM_FRAMES = 1         # Requiere 2 frames consecutivos cerrados (66ms)
DEBOUNCE_FRAMES = 2    # Debounce de 267ms
MIN_EAR_OPEN = 0.30    # EAR mínimo para "abierto" ajustado para ojos asiáticos
EAR_CALIBRATION_MODE = True  # Modo para calibrar el umbral automáticamente
CALIBRATION_SAMPLES = []     # Buffer para calibración inicial
aux_counter = 0        # Contador auxiliar de frames con ojos cerrados
blink_counter = 0      # Contador total de parpadeos
debounce_counter = 0   # Contador para evitar duplicados
last_ear_values = []   # Buffer para suavizado de EAR
...
def eye_aspect_ratio(coordinates):
    """
    Calcula la relación de aspecto del ojo (EAR) usando 6 puntos.
...
def drawing_output(frame, coordinates_left_eye, coordinates_right_eye, total_blinks):
    """
    Dibuja SOLO los puntos de referencia de los ojos (sin relleno transparente).
    """
    output = frame.copy()
    
    # Dibujar puntos de referencia en los ojos con colores vibrantes
    for point in coordinates_left_eye:
        # Punto externo más grande
        cv2.circle(output, tuple(point), 3, (0, 255, 255), -1)  # Cyan exterior
        # Punto interno más pequeño para contraste
        cv2.circle(output, tuple(point), 1, (255, 0, 255), -1)  # Magenta interior
    
    for point in coordinates_right_eye:
        # Punto externo más grande
        cv2.circle(output, tuple(point), 3, (0, 255, 255), -1)  # Cyan exterior
        # Punto interno más pequeño para contraste
        cv2.circle(output, tuple(point), 1, (255, 0, 255), -1)  # Magenta interior
    
    # Dibujar líneas conectando los puntos para visualizar la forma del ojo
    if len(coordinates_left_eye) == 6:
        pts_left = np.array(coordinates_left_eye, np.int32)
        cv2.polylines(output, [pts_left], True, (0, 255, 0), 1)  # Verde
    
    if len(coordinates_right_eye) == 6:
        pts_right = np.array(coordinates_right_eye, np.int32)
        cv2.polylines(output, [pts_right], True, (0, 255, 0), 1)  # Verde
...
def generate_frames():
    """
    Genera frames de video con detección OPTIMIZADA PARA OJOS ASIÁTICOS.
    - Umbrales ajustados para aperturas más estrechas
    - Calibración automática inicial
    ...
    global EAR_THRESH, MIN_EAR_OPEN, CALIBRATION_SAMPLES, EAR_CALIBRATION_MODE
    
    print("=== GENERATE_FRAMES INICIADO (OPTIMIZADO PARA OJOS ASIÁTICOS) ===")
    ...
    with mp_face_mesh.FaceMesh(
        ...
        while streaming_active:
            ...
            # Procesar resultados de FaceMesh
            if results.multi_face_landmarks is not None:
                face_detected = True
                
                for face_landmarks in results.multi_face_landmarks:
                    # Extraer coordenadas del ojo izquierdo
                    for index in INDEX_LEFT_EYE:
                        x = int(face_landmarks.landmark[index].x * width)
                        y = int(face_landmarks.landmark[index].y * height)
                        coordinates_left_eye.append([x, y])
                    
                    # Extraer coordenadas del ojo derecho
