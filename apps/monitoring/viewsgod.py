...
MIN_BLINK_FRAMES = 1        # Mínimo de frames para considerar un parpadeo
MAX_BLINK_FRAMES = 3        # Máximo de frames para un parpadeo normal
DEBOUNCE_FRAMES = 1         # Frames de espera entre parpadeos
                                ...
                                if current_session_id is not None:
                                    try:
                                        ...
                                        BlinkEvent.objects.create(
                                            ...
                                            duration_ms=None  # Puedes calcular duración si tienes lógica de cierre
                                        )
                                    except Exception as e:
                                        print(f"[WARN] No se pudo guardar BlinkEvent: {e}")

                                # Ejemplo de alerta de fatiga: si supera 30 parpadeos en 1 minuto (ajusta según tu lógica)
                                # Aquí puedes poner tu lógica real de alerta
                                        ...
                                        try:
                                            ...
                                            AlertEvent.objects.create(
                                                ...
                                                metadata={
                                                    ...
                                                    'avg_ear': float(current_ear) if current_ear is not None else None
                                                }
                                            )
                                            print(f"[ALERT] ⚠ Alerta de fatiga registrada para sesión {session.id}")
                                        except Exception as e:
